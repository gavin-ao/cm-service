...
public class HttpUtil {
     ...
     * @param json   JSON对象
     */
    public static String doPostSSLJson(String apiUrl, JSONObject json) {
        CloseableHttpClient httpClient = HttpClients.custom().setSSLSocketFactory(createSSLConnSocketFactory()).setConnectionManager(connMgr).setDefaultRequestConfig(requestConfig).build();
        HttpPost httpPost = new HttpPost(apiUrl);
        CloseableHttpResponse response = null;
        String httpStr = null;
     ...
     * @param filePath
     */
    public static String doPostSSLUploadImg(String apiUrl, Map<String, String> param, String fileName, String filePath) {
        CloseableHttpClient httpClient = HttpClients.custom().setSSLSocketFactory(createSSLConnSocketFactory()).setConnectionManager(connMgr).setDefaultRequestConfig(requestConfig).build();
        HttpPost httpPost = new HttpPost(apiUrl);
        CloseableHttpResponse response = null;
        String httpStr = null;
        try {
            ...
            multipartEntityBuilder.addPart(fileName, binFileBody);
            // 设置上传的其他参数
            setUploadParams(multipartEntityBuilder, param);
